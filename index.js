...
app.use("/cdn", async (req, res) => {
    try {
        ...
        // No client headers are forwarded. The proxy fetches the requested
        // authorized CDN URL directly.
...
// Universal fallback for every otherwise-unmatched endpoint.
// All specific routes above keep their normal responses.
// This prevents Express "Cannot GET/POST/..." errors for any path.
...
// CDN proxy status check.
app.get("/cdn-status", (req, res) => {
    res.json({
        code: 200,
        message: "CDN proxy is configured",
        target: "authorized CDN resources only"
    });
});
