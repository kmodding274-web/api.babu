...
// Dynamic Show API: automatically serves ./<id>.json for any show ID.
// Example: /feedservice/v1/shows/505 -> ./505.json
app.get('/feedservice/v1/shows/:id', (req, res, next) => {
    const { id } = req.params;

    // Only allow numeric IDs so arbitrary file paths cannot be requested.
    if (!/^\d+$/.test(id)) {
    ...
    const filePath = require('path').join(__dirname, `${id}.json`);
