...
// Keeps unknown API paths from returning Express "Cannot GET" errors.
// Put this AFTER all specific API routes and BEFORE the CDN middleware.
app.all('/feedservice/*splat', (req, res) => {
    console.log(`Unhandled feedservice endpoint: ${req.method} ${req.originalUrl}`);
...
app.all('/userservice/*splat', (req, res) => {
    console.log(`Unhandled userservice endpoint: ${req.method} ${req.originalUrl}`);
...
app.all('/analytics/*splat', (req, res) => {
    console.log(`Unhandled analytics endpoint: ${req.method} ${req.originalUrl}`);
...
// Open: /cdn-test
app.get("/cdn-test", async (req, res) => {
    try {
        const testUrl = "https://cdn.storymax.app/";
        ...
        console.log(`CDN TEST: ${upstream.status} ${upstream.headers.get("content-type") || ""}`);

        res.status(upstream.status).json({
            code: upstream.status,
            message: "CDN request reached successfully",
